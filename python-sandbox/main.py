...
def get_product_name(product):
  splited_product = product.split(' - ')
  if(len(splited_product)==0): return None
  else:
    return ' - '.join(splited_product[1:])
...
def create_or_get_container(container_name):
...
if __name__ == "__main__":
    connection_string = os.getenv('AZ_CONN')
    if connection_string:
        blob_service_client = BlobServiceClient.from_connection_string(conn_str=connection_string)
        if(blob_service_client): print("Serviço criado com sucesso!")

        testfiles_container = blob_service_client.get_container_client("testfiles")
        ...
        container_client = create_or_get_container(container_name)
